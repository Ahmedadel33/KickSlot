...
export default function OwnerLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      const res = await axios.post("http://localhost:8000/admin/login", { // أو الـ endpoint الجديدة للـ owner
        email: form.email.trim(),
        password: form.password,
      });
      localStorage.setItem("ownerToken", res.data.token);
      localStorage.setItem("ownerEmail", form.email.trim());
    ...
    <div className={styles.page}>
      ...
      <div className={styles.wrapper}>
        ...
        <div className={styles.brandTop}>
          ...
          <p className={styles.brandSub}>Owner Panel</p>
        </div>

        <div className={styles.card}>
          <h2 className={styles.cardTitle}>Owner Access</h2>
          <p className={styles.cardSub}>Sign in to manage your pitches</p>
